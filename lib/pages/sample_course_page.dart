...
import 'package:myanmar_educational/bloc/sample_course_page_bloc.dart';
...
class SampleCoursePage extends StatelessWidget {
  final CourseVO course;
  const SampleCoursePage({super.key, required this.course});

  @override
  Widget build(BuildContext context) {
    return ChangeNotifierProvider<SampleCoursePageBloc>(
      create: (_) => SampleCoursePageBloc(course.courseID ?? ''),
      child: SingleChildScrollView(
        scrollDirection: Axis.vertical,
        child: Container(
          height: MediaQuery.of(context).size.height,
          padding: const EdgeInsets.only(top: k20px, left: k20px, right: k20px),
          child: Column(
            ...
            children: [
              ...
              SizedBox(
                ...
                child: Selector<SampleCoursePageBloc, List<VideoVO>?>(
                    selector: (_, bloc) => bloc.getSampleVideoList,
                    builder: (_, sampleVideoList, child) =>(sampleVideoList == null) ? Center(child: EasyTextWidget(text: 'There is no course video', textColor: kPrimaryColor, textSize: k15px, fontWeight: FontWeight.normal),) :
                    ListView.separated(
                      scrollDirection: Axis.vertical,
                            ...
                            children: [
                              ...
                              Column(
                                ...
                                children: [
                                  ...
                                  DurationView(duration: sampleVideoList[index].videoDuration ?? '')
                                ],
                              )
                            ],
                          ),
                      ...
                      itemCount: sampleVideoList.length ?? 0,
                    )
                ),
              )
            ],
...
class DurationView extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return EasyTextWidget(text: "Duration: ${context.read<SampleCoursePageBloc>().formatDuration(double.parse(duration))}", textColor: kWhite, textSize: k15px, fontWeight: FontWeight.normal)
    ;
  }
}
